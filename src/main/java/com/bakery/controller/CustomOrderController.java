...
public class CustomOrderController {
    ...
    public String customOrderForm(@AuthenticationPrincipal UserDetails userDetails, Model model) {
        ...
        model.addAttribute("userName", user.getName());// sends data to the front end
    ...
    public String submitCustomOrder(
        ...
        redirectAttributes.addFlashAttribute("orderId", order.getId());// temperory data stored only for the next page
    ...
    //handles cancelling a custom order
    @PostMapping("/{id}/cancel")
    public String cancelCustomOrder(

            @AuthenticationPrincipal UserDetails userDetails,

            @PathVariable int id,// gets the order id from the url
        ...
        try {
            ...
            //calls the service method to cancel the order
            customOrderService.cancelOrderForUser(id, user);
            ...
            //shows the error message if cancelling fails
            redirectAttributes.addFlashAttribute(
                    "error",
                    e.getMessage()
            );
        ...
        //redirects back to my orders page
        return "redirect:/customer/custom-order/my-orders";
    }
}
